...
const Services = () => {
    return (
        <div className="bg-white text-black pt-12 pb-20 px-10 sm:px-12 lg:px-12">

            {/* Header */}
            <div className="text-center mb-8 pt-0">
                <p className="font-script text-4xl text-gray-400 mb-2">Precios Transparentes</p>
                <h1 className="text-4xl md:text-6xl font-oswald font-bold uppercase tracking-tighter mb-6">
                    Carta de Servicios
                </h1>
                        ...
                        {categories[0].items.map((item) => (
                            <div key={item.name} className="flex flex-col sm:flex-row justify-between items-baseline group py-6 border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors px-4 rounded-lg">
                                <div className="flex-1 pr-8">
                                    <h3 className="text-xl font-oswald font-bold text-black uppercase tracking-wide">
                                        {item.name}
                                    </h3>
                                    <p className="text-sm text-gray-500 mt-2 font-sans">{item.desc}</p>
                                </div>
